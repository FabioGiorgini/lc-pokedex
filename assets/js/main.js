...
const cardContainer = document.getElementById('poke-list');

//al caricamento della pagina ciclo la lista dei pokemon per generare le diverse card
list.pokemon.forEach((pokemon, index) => {
   generatorCard (pokemon)

   //una volta generate le card, prendo tutte le div .type per aggiungerci i badge del tipo pokemon
...
searchButton.addEventListener('click', function() {
  ...
  for(let i = 0; i < list.pokemon.length; i++) {
    //controllo che l'input dell'utente sia incluso nei diversi nomi dei pokemon e nel caso lo aggiungo all'array pokemonFound
...
//al caricamento della pagina inserisco anche il numero di pokemon nell'array
shownCardsContainer.innerHTML = `Pokemon trovati: ${list.pokemon.length} / ${list.pokemon.length}`

// FUNCTIONS

function generatorCard (pokemon) {
  //nella lista aggiungo i singoli li con all'interno la card e le informazioni come da screen
  cardContainer.innerHTML += `
...
/* <span class="me-2 badge bg-secondary"> </span> */